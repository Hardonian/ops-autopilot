...
function preflight(bundle: ReturnType<typeof analyze>['jobRequestBundle']) {
  ...
  for (const request of bundle.requests) {
    ...
    if (!request.policy.requires_policy_token) {
      errors.push('job request missing policy token requirement');
    }
    if (!request.policy.requires_approval) {
      errors.push('job request missing approval requirement');
    }
  }
...
describe('JobForge integration fixtures', () => {
  it('matches stable fixtures', () => {
    ...
    expect(reportMarkdownFixture).toContain('# Growth Autopilot Report');

    expect(result.reportEnvelope.report.report_id).toBe('00000000-0000-0000-0000-000000000000');
    expect(result.reportEnvelope.report.module.name).toBe('growth-autopilot');
    expect(result.reportEnvelope.report.generated_at).toBe('2000-01-01T00:00:00.000Z');
    expect(result.reportEnvelope.report.findings.seo_issues).toBe(1);
