import { z } from 'zod';
import { ISODateTimeSchema, JSONValueSchema, PrioritySchema, SemVerSchema } from './core.js';
...
export function batchJobRequests(requests: JobRequest[]): JobRequestBatch {
  ...
  const totalCredits = requests.reduce((sum, r) => sum + (r.cost_estimate?.credits ?? 0), 0);
