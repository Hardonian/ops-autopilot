...
const FORBIDDEN_PATTERNS = [
  ...
  {
    pattern: /require\(['"]bullmq['"]\)|import.*from\s+['"]bullmq['"]/,
    name: 'bullmq',
    type: 'queue',
  },
  {
    pattern: /require\(['"]bee-queue['"]\)|import.*from\s+['"]bee-queue['"]/,
    name: 'bee-queue',
    type: 'queue',
  },
  {
    pattern: /require\(['"]agenda['"]\)|import.*from\s+['"]agenda['"]/,
    name: 'agenda',
    type: 'queue',
  },
  ...
  // Scheduler libraries
  {
    pattern: /require\(['"]node-cron['"]\)|import.*from\s+['"]node-cron['"]/,
    name: 'node-cron',
    type: 'scheduler',
  },
  {
    pattern: /require\(['"]cron['"]\)|import.*from\s+['"]cron['"]/,
    name: 'cron',
    type: 'scheduler',
  },
  {
    pattern: /require\(['"]node-schedule['"]\)|import.*from\s+['"]node-schedule['"]/,
    name: 'node-schedule',
    type: 'scheduler',
  },
  ...
  {
    pattern: /require\(['"]express['"]\)|import.*from\s+['"]express['"]/,
    name: 'express',
    type: 'server',
  },
  {
    pattern: /require\(['"]fastify['"]\)|import.*from\s+['"]fastify['"]/,
    name: 'fastify',
    type: 'server',
  },
  ...
  // Secret management
  {
    pattern: /process\.env\.[A-Z_]*(?:SECRET|PASSWORD|TOKEN|KEY|CREDENTIAL)/,
    name: 'env-secret',
    type: 'secret',
  },
  {
    pattern: /require\(['"]dotenv['"]\).*config\s*\(\s*\{[^}]*secret/i,
    name: 'dotenv-secret',
    type: 'secret',
  },
  ...
  {
    pattern: /axios\.[getpostputdelete]+\s*\(\s*['"]https?:/,
    name: 'axios-direct',
    type: 'connector',
  },
...
export async function auditRunnerless(repoPath: string): Promise<AuditResult> {
  ...
  const checks: CheckResult[] = [
    ...
    {
      name: 'No forbidden libraries',
      passed: violations.filter(v => v.severity === 'error').length === 0,
      details: `${filesAnalyzed} files analyzed`,
    },
